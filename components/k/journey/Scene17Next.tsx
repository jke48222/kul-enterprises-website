...
/** His words and the record's labels, edited at /admin. The register rows
 *  themselves come from the spine: they are the archive, not copy. */
export type Scene17Copy = {
  summation: string[];
  turnA: string;
  turnB: string;
  silenceWord: string;
...
export default function Scene17Next({ copy }: { copy: Scene17Copy }) {
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-[1296px] px-5 pt-[16svh] md:px-10 lg:px-24">
        ...
        <Margined label={copy.silenceWord} className="mt-[12svh]">
        ...
        {/* [E] THE TWO DOORS. The story has opened the front door; these are
            the handles. */}
        <Margined label={copy.continueWord} className="mt-[12svh]">
          <div>
      ...
      <nav aria-label="Plate index" className="mx-auto mt-[14svh] w-full max-w-[1296px] px-5 md:px-10 lg:px-24">
        ...
        <div className="sticky top-0 z-10 flex items-baseline justify-between gap-6 border-b py-4 font-display text-[11px] font-medium uppercase tracking-[0.1em]" style={{ background: SCENE.from, borderColor: "color-mix(in srgb, currentColor 20%, transparent)" }}>
          ...
          <span className="hidden md:inline">{copy.indexTitle}</span>
        ...
        <Margined label={copy.colophonWord} className="mt-[8svh]">
