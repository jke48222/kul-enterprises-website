...
/**
 * QuoteStrip — v3 port (v2 §3.16). The one-row booking strip, wired to
 * /api/quote with the EXACT contract field names (origin, destination,
 * freightType, pickupDate, contact) + the `botcheck` honeypot via the
 * shared FormShell machinery. Focus choreography: micro label lifts, a
 * 1.5px currentColor underline scales in (never gold). The circular gold
 * submit arrow is the section's 2nd gold.
 */
...
export function QuoteStrip({
  ...
  const cell =
    "group relative px-0 py-4 md:py-5 md:px-[clamp(12px,1.2vw,24px)] md:first:pl-0";
  const label = `block text-micro uppercase ${labelColor} transition-opacity duration-200 group-focus-within:opacity-100`;
  const input = `peer mt-2 block w-full appearance-none rounded-none bg-transparent text-[17px] ${inputColor} outline-none`;
  ...
  return (
    <div data-ground={ground}>
      <Heading
        className={`text-h3 font-semibold ${ink ? "text-paper" : "text-ink"}`}
      >
        {heading}
      </Heading>
      ...
      <AnimatePresence mode="wait" initial={false}>
        {state === "success" ? (
          ...
          >
            <p
              className={`max-w-[30ch] text-h3 font-semibold ${ink ? "text-paper" : "text-ink"}`}
            >
          ...
          >
            ...
            >
              ...
              {/* SUBMIT — circular gold arrow, the section's 2nd gold. */}
            ...
            <p className={`mt-4 text-micro uppercase tabular-nums ${microMuted}`}>
