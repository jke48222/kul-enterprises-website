        ...
        fields: [
          {
            ...
            fields: [
              ...
              text("leadTime", "Lead time", "For example: 24 to 48 hrs."),
              {
                type: "boolean",
                name: "leadTimeNoPrefix",
                label: "Print it without the words 'Lead time' in front",
                description:
                  "For an entry that is not a time, like By contract, so the page does not read 'Lead time By contract'.",
              },
              {
                type: "object",
                name: "dimensions",
         ...
         ============================================================ */
      {
        ...
        fields: [
          ...
          group(
            ...
            "The menu panels",
            [
              group("services", "Services panel", [
                text("compareHeading", "The end card's first line", "On the card at the end of the row of photographs."),
                text("compareLabel", "The gold words under it"),
              ]),
              group("drivers", "Drivers panel", [
                text("label", "The small capital label"),
                longText(
                  "status",
                  "The status line",
                  "The largest line in the panel. It says whether applications are open, and it must stay true to the Drivers page.",
                ),
                text("applyLabel", "The gold link beside it"),
                {
                  type: "object",
                  name: "facts",
                  label: "The three facts about the seat",
                  description: "Three is what the row is built for. Each must match what the Drivers page says.",
                  list: true,
                  ui: { itemProps: (item) => ({ label: item?.term || "Fact" }) },
                  fields: [text("term", "The gold heading"), longText("detail", "The sentence under it")],
                },
              ]),
              group("safety", "Safety panel", [
                text("label", "The small capital label"),
                text("usdotLabel", "USDOT label", "The number itself comes from Business Facts."),
                text("mcLabel", "MC label", "The number comes from Business Facts too, and the pair only shows while an MC number is entered there."),
                longText("note", "The sentence beside the numbers"),
                text("verifyLabel", "The gold verify link", "Where it goes is set once, on the footer's verify button, and this link follows it."),
                text("safetyLabel", "The grey link under it"),
              ]),
              group("packet", "Carrier Packet panel", [
                text("label", "The small capital label"),
                {
                  type: "object",
                  name: "documents",
                  label: "The manifest",
                  description: "One row per document in the packet, with the small words on the right saying how it arrives.",
                  list: true,
                  ui: { itemProps: (item) => ({ label: item?.name || "Document" }) },
                  fields: [text("name", "Document"), text("state", "The small words on the right")],
                },
                text("requestLabel", "The gold link"),
              ]),
              group("about", "About panel", [
                text("label", "The small capital label", "This one sits on the right of its panel, by client direction."),
                image("lockup", "The colour lockup", "The one file in the kit with a genuinely transparent background. It is resized, never trimmed."),
                text("line", "The line beside it"),
                {
                  type: "object",
                  name: "facts",
                  label: "The three facts",
                  description: "Three is what the row is built for. Keep each one true the day it is written.",
                  list: true,
                  ui: { itemProps: (item) => ({ label: item?.term || "Fact" }) },
                  fields: [text("term", "The grey heading"), text("detail", "The fact", TOKENS)],
                },
              ]),
              group("journey", "Journey panel", [
                text("label", "The small capital label"),
                text("line", "The line about the film"),
                text("watchLabel", "The gold link beside it"),
                longText("roadAheadLine", "The line about the plan"),
                text("roadAheadLabel", "The gold link beside that"),
              ]),
              group("contact", "Contact panel", [
                text("label", "The small capital label"),
                text("phoneLabel", "Above the phone number", "The number itself comes from Business Facts."),
                text("emailLabel", "Above the email address"),
                text("basedLabel", "Above the city"),
                text("quoteLabel", "The gold link on the right"),
              ]),
            ],
            "What drops down under each link on the wide bar. The numbers, the phone number and the email in these panels come from Business Facts, so they cannot go stale here.",
          ),
          group("footer", "The footer", [
            ...
            text(
              "verifyHref",
              "Verify button link",
              `One link, used twice: the footer's verify button and the Safety menu panel both follow it. ${TOKENS}`,
            ),
