...
/**
 * JOURNEY, THE HALF THAT DRAWS.
 *
 * app/journey/page.tsx fetches on the server and hands the result down. This half
...
const HELD_CHAPTERS = CHAPTERS.slice(1, 4).map((chapter) => ({
  n: chapter.n,
  title: chapter.title,
  ground: GROUND_CLASS[chapter.ground],
  lines: chapter.lines.slice(0, -1),
  held: chapter.lines[chapter.lines.length - 1],
}));
 ...
 *
 * THE ORDER IS THE ORDER OF THE LIGHT AND IT IS NOT A PREFERENCE. Two things
 * were fixed here after looking at the sequence as a contact sheet, and neither
 * was visible one frame at a time. Both are recorded because the CMS now lets
 * anybody drag these into a new order:
 *
 ...
 * on its own is still worth its place here.
 *
 * ALT TEXT IS EMPTY ON PURPOSE, which is why the CMS stores these as bare
 * pictures with no description field beside them. This is one decorative
 * field, described once by the sentence held over it, and not eighteen
 * separate things for a screen reader to read out.
 */
...
export default function JourneyView(props: TinaPage<{ journeyPage: unknown }>) {
  ...
  const journey = data.journeyPage as unknown as Content;
  ...
  /**
   * The three chapters with no photograph, taken from the shared chapter list.
   *
   * THEY ARE DERIVED, NOT WRITTEN OUT AGAIN. This file used to hold its own copy
   * of chapters 02 to 04 with the same words typed a second time, which was
   * survivable only while both copies were edited by the same developer in the
   * same commit. It is not survivable now the words are in the CMS: the client
   * would edit the sleeve and leave the section under it saying something else.
   *
   * The shape of a chapter here is every line but the last, then the last one on
   * its own as the held statement. That is exactly how the sleeves are written in
   * content/journey.json, where the closing line always comes last.
   *
   ...
   */
  const ROAD = journey.chapterFive.frames;
  const SCATTER = journey.contactSheet.tiles.map((src) => ({ src, alt: "" }));
  ...
  return (
    <>
      ...
      <PinnedStatement text={fill(journey.pinnedStatement)} />
      ...
      <section id="chapter-01" className="scroll-mt-24 bg-k-warm px-6 py-24 md:px-12 lg:px-24 lg:py-32">
        <div className="mx-auto flex max-w-[1248px] flex-col gap-14 lg:flex-row lg:items-end lg:gap-24">
          <Reveal className="lg:shrink-0">
            <Image
              src={journey.chapterOne.image}
              alt={fill(journey.chapterOne.imageAlt)}
              width={1291}
              height={1920}
          ...
          <div className="flex flex-col lg:pb-4">
            <span className="font-text text-k-micro uppercase tabular-nums text-k-gold">
              {fill(journey.chapterOne.eyebrow)}
            </span>
            <h2
              data-tina-field={tinaField(journey.chapterOne, "heading")}
              className="pt-4 font-display text-k-d2 font-black text-k-ink"
            >
              {fill(journey.chapterOne.heading)}
            </h2>
            <div className="flex max-w-[52ch] flex-col gap-5 pt-8">
              {journey.chapterOne.paragraphs.map((paragraph) => (
                <p
                  key={paragraph}
            ...
            {/* The heaviest line in the whole story, given its own weight and
                ...
                It is a separate field in the CMS rather than the last of the
                paragraphs above precisely so it cannot lose that. */}
            <RuleDraw className="mt-10 max-w-[400px]" />
            <p className="max-w-[24ch] pt-8 font-display text-k-d3 font-black text-k-ink">
              {fill(journey.chapterOne.closingLine)}
            </p>
          </div>
        </div>
      </section>
        ...
        >
          <div className="mx-auto flex w-full max-w-[1248px] flex-col gap-10 lg:flex-row lg:gap-24">
            <Reveal variant="settle" className="lg:w-[240px] lg:shrink-0">
              ...
              <h2
              data-tina-field={tinaField(chapter, "title")}
              className="pt-4 font-display text-k-d3 font-black text-k-on-dark"
            >
              {fill(chapter.title)}
            </h2>
            </Reveal>
      ...
      <section id="chapter-05" className="scroll-mt-24 bg-k-void">
        ...
        <div className="relative h-[54svh] overflow-hidden">
          <HeroVideo
            name={journey.chapterFive.video}
            poster={journey.chapterFive.poster}
        ...
        <div className="mx-auto w-full max-w-[1248px] px-6 pb-28 pt-16 md:px-12 lg:px-24">
          <span className="font-text text-k-micro uppercase tabular-nums text-k-gold-lit">
            {fill(journey.chapterFive.eyebrow)}
          </span>
          <Reveal className="pt-4">
            <h2
              data-tina-field={tinaField(journey.chapterFive, "heading")}
              className="max-w-[16ch] font-display text-k-d1 font-black text-k-on-dark"
            >
              {fill(journey.chapterFive.heading)}
            </h2>
          </Reveal>
          <p className="max-w-[52ch] pt-7 font-text text-k-lede text-k-on-dark-soft">
            {fill(journey.chapterFive.body)}
          </p>
        </div>
      </section>
      ...
      <JourneyScatter
        tiles={SCATTER}
        eyebrow={fill(journey.contactSheet.eyebrow)}
        statement={fill(journey.contactSheet.statement)}
      />
      ...
      <section id="chapter-06" className="scroll-mt-24 bg-k-paper px-6 py-28 md:px-12 lg:px-24 lg:py-36">
        <div className="mx-auto max-w-[1248px]">
          <div className="flex flex-col gap-10 lg:flex-row lg:gap-24">
            <Reveal variant="settle" className="lg:w-[240px] lg:shrink-0">
              <span className="font-text text-k-micro uppercase tabular-nums text-k-gold">
                {fill(journey.chapterSix.eyebrow)}
              </span>
              <h2
              data-tina-field={tinaField(journey.chapterSix, "heading")}
              className="pt-4 font-display text-k-d3 font-black text-k-ink"
            >
              {fill(journey.chapterSix.heading)}
            </h2>
            </Reveal>

            {/* The footnote is a separate field rather than a third paragraph
                because it is set a size down and in the faint tone: it is the
                registration detail under the story, not another line of it. */}
            <div className="flex max-w-[54ch] flex-1 flex-col gap-6">
              {journey.chapterSix.paragraphs.map((paragraph, i) => (
              ...
              <Reveal index={journey.chapterSix.paragraphs.length}>
                <p className="font-text text-k-body text-k-ink-faint">
                  {fill(journey.chapterSix.footnote)}
                </p>
              </Reveal>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6 pt-20 md:gap-10">
            <Reveal>
              <Image
                src={journey.chapterSix.imageWide}
                alt={fill(journey.chapterSix.imageWideAlt)}
                width={1920}
                height={1440}
            ...
            <Reveal index={1} className="lg:w-[62%] lg:self-end">
              <Image
                src={journey.chapterSix.imageInset}
                alt={fill(journey.chapterSix.imageInsetAlt)}
                width={1920}
                height={1440}
              ...
              <p className="pt-4 font-text text-k-small text-k-ink-faint">
                {fill(journey.chapterSix.imageInsetCaption)}
              </p>
            </Reveal>
          </div>
        </div>
      ...
      <section className="bg-k-surface px-6 py-24 md:px-12 lg:px-24">
        <div className="mx-auto flex max-w-[1248px] flex-col gap-12 lg:flex-row lg:gap-24">
          <Reveal variant="settle" className="lg:w-[240px] lg:shrink-0">
            <h2
              data-tina-field={tinaField(journey.close, "heading")}
              className="font-display text-k-d3 font-black text-k-ink"
            >
              {fill(journey.close.heading)}
            </h2>
          </Reveal>
          <div className="flex max-w-[62ch] flex-1 flex-col gap-6">
            ...
            {journey.close.paragraphs.map((paragraph) => (
              <p
