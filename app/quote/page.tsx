...
import pageJson from "@/content/pages/quote.json";
import QuoteView from "./quote-view";

/**
 * QUOTE, THE SERVER HALF.
 *
 * Fetches the content and owns the page metadata; app/quote/quote-view.tsx draws it.
 ...
 *
 * Every word is in content/pages/quote.json and is edited at /admin. The import
...
export default async function QuotePage() {
  const page = await tinaPage(
    "Quote page",
    () => client.queries.quotePage({ relativePath: "quote.json" }),
    // The shape the query would have returned, built from the file on disk.
    { quotePage: pageJson } as never,
  );

  return <QuoteView {...page} />;
}
